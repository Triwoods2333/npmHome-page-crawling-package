...
class NPM_Feature_Extractor:
    ...
    def __init__(self) :
        ...
        # dangerous token  F:\git_workspace\cross-language-detection-artifacts\scripts\feature_extraction\resources
        with open('C:/Users/97091/Desktop/getgithub/resources/dangerous_tokens.json', 'r') as file:
    ...
    def extract_features(self, path: str) -> pd.DataFrame:
        ...
        final_df.to_csv(r"C:/Users/97091/Desktop/getgithub/npm_feature_extracted.csv", encoding='utf-8', index=False)
        return final_df
    ...
    def extract_feature_from_package_json(self) -> pd.DataFrame:
        ...
        for i in range(len(files_path)):
            ...
            if files_path[i].split('\\')[-1] == "package.json":
                # initialize the list for the puntuactions and operators token 
                operator=[]
                punctuation=[]
                other=[]
        ...
        db['strings']=strings
        db['code']=code
        #db['Number of words'] = db["code"].apply(lambda n: len(n.split()))
        # print(db.head())
    ...
    def count_package_files_extension(self) -> pd.DataFrame:
        ...
        # fill Nan with 0
        db[f] = db[f].fillna(0).infer_objects(copy=False)
        # order the column 
        db=db[extensions]
    ...
    def extraction(self,database,alphabetic_string,base_string,alphabetic_id,base_id):
        ...
        database[f] = database[f].fillna(0).infer_objects(copy=False)
        # reset index 
